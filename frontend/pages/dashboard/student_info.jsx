...
export default function StudentInfo() {
    ...
    if (sig) {
      ...
      (async () => {
        try {
          ...
          const isValid = await verifySignature(studentIdFromUrl, signature);
  ...
  // React Query hook with real-time updates (authenticated access)
  ...
  const isLoadingStudent = hasAuthToken ? studentLoading : publicStudentLoading;
  const studentErrorData = hasAuthToken ? studentError : publicStudentError;
  ...
  useEffect(() => {
    if (studentErrorData) {
      if (studentErrorData.response?.status === 404) {
        ...
        // Redirect to student_not_found if public access
        if (!hasAuthToken) {
          router.push('/student_not_found');
        }
      } else {
        console.log('❌ Student Info Page - Error fetching student:', {
          searchId: searchId || studentId,
          error: studentErrorData.message,
          timestamp: new Date().toLocaleTimeString()
        });
      ...
      if (currentStudent && !studentErrorData) {
        setStudentDeleted(false);
        setError("");
      }
    }
  }, [studentErrorData, searchId, studentId, currentStudent, hasAuthToken, router]);
  ...
  const getWeekAttendance = (weekNumber) => {
    if (!currentStudent || !currentStudent.weeks) return { attended: false, hwDone: false, quizDegree: null, message_state: false, lastAttendance: null };
  ...
  return (
    <>
      {hasAuthToken ? <Header /> : <PublicHeader />}
      <div style={{ 
        ...
        {/* Only show Title when authenticated */}
        {hasAuthToken && (
          <Title>
        ...
        {/* Only show search form when authenticated */}
        {hasAuthToken && (
          <div className="form-container">
            <form onSubmit={handleIdSubmit} className="fetch-form">
              <input
                ...
                required
              />
              <button type="submit" className="fetch-btn" disabled={isLoadingStudent}>
                {isLoadingStudent ? "Loading..." : "🔍 Search"}
              </button>
            </form>
        ...
        {/* Welcome message for public access */}
        {!hasAuthToken && currentStudent && !studentDeleted && (
          <div style={{
            ...
            marginBottom: "20px",
            textAlign: "center",
            boxShadow: "0 8px 32px rgba(31, 168, 220, 0.2)",
            border: "1px solid rgba(255,255,255,0.2)"
          }}>
            <h2 style={{
              fontSize: "1.8rem",
          ...
          <div className="info-container">
            <div className="student-details">
              ...
              <div className="detail-item">
                ...
                <div className="detail-value">{currentStudent.grade}</div>
              </div>
              <div className="detail-item">

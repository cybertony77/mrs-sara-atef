  ...
  const handleGenerate = async (e) => {
    ...
    if (isFullPhone) {
      if (allStudents) {
        ...
        if (matchingStudents.length === 1) {
          ...
          const link = await generatePublicStudentLink(student.id.toString());
    ...
    if (isAllDigits) {
      ...
      if (allStudents) {
        ...
        if (byId) {
          const link = await generatePublicStudentLink(String(byId.id));
          setGeneratedLink(link);
        ...
        if (matchingStudents.length === 1) {
          ...
          const link = await generatePublicStudentLink(foundStudent.id.toString());
          setGeneratedLink(link);
      ...
      const link = await generatePublicStudentLink(searchTerm);
      setGeneratedLink(link);

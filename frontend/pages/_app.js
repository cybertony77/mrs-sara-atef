...
function Preloader() {
  return (
    <div style={{
      ...
      width: '100%',
      height: '100%',
      background: 'linear-gradient(300deg,rgba(209, 96, 158, 1) 0%, rgba(63, 187, 212, 1) 100%);',
      display: 'flex',
      alignItems: 'center',
      ...
      <div style={{
        ...
        <div style={{
          ...
          borderTop: '4px solid rgb(31, 38, 51)',
          borderRadius: '50%',
...
export default function App({ Component, pageProps }) {
  ...
  // Check if current route is public student_info page with HMAC signature
  const isPublicStudentInfoPage = useMemo(() => {
    if (router.pathname === '/dashboard/student_info' && router.query.sig) {
      return true;
    }
    return false;
  }, [router.pathname, router.query.sig]);

  // Define public pages using useMemo to prevent recreation on every render
  const publicPages = useMemo(() => {
    const basePages = ["/", "/404", "/contact_developer", "/student_not_found", "/student_info"];
    // Add /dashboard/student_info if it has HMAC signature
    if (isPublicStudentInfoPage) {
      return [...basePages, "/dashboard/student_info"];
    }
    return basePages;
  }, [isPublicStudentInfoPage]);
  ...
  useEffect(() => {
    // Allow public access to student_info page with HMAC signature
    if (isPublicStudentInfoPage) {
  ...
  }, [isLoading, isAuthenticated, router.pathname, publicPages, router, isPublicStudentInfoPage]);
  ...
  useEffect(() => {
    const fetchSubscription = async () => {
      // Skip subscription check for public student_info page with HMAC
      if (!isAuthenticated || publicPages.includes(router.pathname) || isPublicStudentInfoPage) {
  ...
  }, [isAuthenticated, router.pathname, publicPages, isPublicStudentInfoPage]);
  ...
  // Allow public student_info page with HMAC to bypass loading
  if ((isLoading || (isAuthenticated && isLoadingSubscription && !publicPages.includes(router.pathname)) || isRouteChanging) && !isPublicStudentInfoPage) {
    return <Preloader />;
  }

  // Show redirect to login preloader if redirecting due to unauthorized access
  // Don't show for public student_info page with HMAC
  if (showRedirectToLogin && !isPublicStudentInfoPage) {
  ...
  // Allow public student_info page with HMAC to bypass this check
  if (!isAuthenticated && !publicPages.includes(router.pathname) && !isPublicStudentInfoPage) {
    return <Preloader />;
  }

  // Only show Header/Footer if user is authenticated (or public student_info with HMAC)
  if (!isAuthenticated && !isPublicStudentInfoPage) {
    return (
      ...
      <ErrorBoundary>
        <MantineProvider>
          <div className="page-container" style={{ 
            ...
            {router.pathname !== "/" && router.pathname !== "/dashboard/student_info" && <Header />}
